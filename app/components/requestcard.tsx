...
export const RequestCard = ({ request }: { request: RequestProps }) => {
  ...
  useEffect(() => {
    setTruncatedDescription(limitText(request?.description ?? "", 30));
  }, [request]);

  return (
    <>
      <div
        className="shadow-xl border-2  h-auto w-[295px] mb-4 rounded-[10px] hover:transform  transition-transform duration-300 ease-in-out "
      ...
      >
        ...
        <div className="flex flex-row">
          ...
          <div className="flex flex-col justify-center ml-2">
            ...
            <p className="text-[12px] text-gray-500">{request?.requesterCity}</p>
          </div>
        ...
        {showOptions && (
          <div className="flex flex-col justify-center items-center mt-4">
            {request?.status === "Pending" ? (
              <button className="text-center bg-orange-500 text-white mb-4 rounded-full h-[35px]  w-[250px] hover:bg-white hover:text-yellow-500 hover:border-[2px] hover:border-yellow-500 hover:ease-in-out duration-300">
                Edit Request
              </button>
            ) : null}
            ...
            {request?.status === "Pending" || request?.status === "OnGoing" ? (
              <button
                className={`text-center bg-rose-500 text-white rounded-full h-[35px]  w-[250px] hover:bg-white hover:text-rose-500 hover:border-[2px] hover:border-rose-500 hover:ease-in-out duration-300 ${
            ...
            {request?.status === "Pending" ? (
              ...
              >
                <button
                  ...
                  } text-white mt-4 rounded-full h-[35px]  w-[250px] hover:bg-white hover:text-blue-500 hover:border-[2px] hover:border-blue-500 hover:ease-in-out duration-300`}
                >
                  {applications.length > 0
        ...
        <div className="flex flex-col justify-center items-center mt-4 mb-4">
          <button
            className="text-center bg-green-500 text-white  mb-0 rounded-full h-[35px]  w-[250px]  hover:bg-white hover:text-green-500 hover:border-[2px] hover:border-green-500 hover:ease-in-out duration-300"
